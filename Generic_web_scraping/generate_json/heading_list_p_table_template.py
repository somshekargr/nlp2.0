...
def get_heading_list_p_table_data(soup_data, title_tag, title_tag_data, content_tag, content_tag_data):
    dict_data = []
    ...
    for res_data in result_data:
        ...
        if para_data:
            for p_d in main_content.findAll('p'):
                if p_d == para_data:
    ...
    for head_data in main_content.findAll(re.compile("^h[1-6]$")):
        if head_data.text.strip():
...
#     __file__), '..', 'indianbank_data', 'indianbank_138.txt'), encoding="utf8"), 'html.parser')
