...
def get_figcaption_figure_heading_img_p_table_data(soup_data, title_tag,title_tag_data, content_tag,content_tag_data):
    """
    ...
    for heading_data in main_content.findAll(re.compile("^h[1-6]$")):
        dict_data.append(generate_dict_data(title_data, '', heading_data))
        heading_data.decompose()

    for para_data in main_content.findAll('p'):
        dict_data.append(generate_dict_data(title_data, '', para_data))
        para_data.decompose()

    return dict_data

# soup = BeautifulSoup(open(os.path.join(os.path.dirname(
#     __file__), '..', 'indianbank_data', 'indianbank_55.txt'), encoding="utf8"), 'html.parser')
# result_data = get_figcaption_figure_heading_img_p_table_data(soup, 'innerheading', 'mainContent')
